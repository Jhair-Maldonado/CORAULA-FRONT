      ...
      <div className="max-w-5xl w-full mx-auto px-6 md:px-8 pb-12 -mt-12 relative z-20 flex-1 flex flex-col gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-line p-5 md:p-6 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-2xl bg-accent/10 text-accent border border-accent/20 flex items-center justify-center font-bold text-xl shrink-0">
              <BookOpen01Icon size={28} />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-bold uppercase tracking-wider bg-neutral text-ink px-2 py-0.5 rounded border border-line">
                  {curso.codigo}
                </span>
              ...
              <h1 className="text-xl md:text-2xl font-bold text-ink tracking-tight mt-1">
                {curso.nombre}
              </h1>
          ...
          <div className="md:col-span-5 flex flex-col gap-6">
                ...
                <div className="p-4 rounded-xl bg-neutral/40 border border-line flex flex-col gap-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="w-10 h-10 rounded-xl bg-accent text-white font-bold text-xs flex items-center justify-center shrink-0 uppercase">
                        {archivo.tipo}
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs font-bold text-ink truncate">{archivo.nombre}</p>
            ...
            {curso.temarioResumen && (
              <div className="bg-white rounded-2xl border border-line p-5 shadow-sm flex flex-col gap-3">
                <h3 className="text-xs font-bold text-ink uppercase tracking-wider">Unidades Temáticas</h3>
                <div className="flex flex-col gap-2">
                  {curso.temarioResumen.map((tema, i) => (
                    <div key={i} className="flex items-start gap-2.5 p-2.5 rounded-lg bg-neutral/30 border border-line/40 text-xs font-bold text-ink">
                      <span className="w-5 h-5 rounded bg-accent/10 text-accent text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5">
                        {i + 1}
                      </span>
                      <span>{tema}</span>
                    </div>
            ...
            <div className="bg-white rounded-2xl border border-line p-5 md:p-6 shadow-sm flex flex-col gap-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 rounded-xl bg-neutral/40 border border-line/60 flex flex-col">
                  ...
                  <span className="text-lg font-bold text-ink mt-0.5">{curso.frecuenciaSemanal} días/sem</span>
                </div>

                <div className="p-3 rounded-xl bg-neutral/40 border border-line/60 flex flex-col">
                  <span className="text-[9px] font-bold text-muted uppercase tracking-wider">Horas Lectivas</span>
                  <span className="text-lg font-bold text-ink mt-0.5">{curso.horasTotalesSemana} hrs/sem</span>
                </div>
                ...
                <div className="p-3 rounded-xl bg-accent/10 border border-accent/20 flex flex-col">
                  ...
                  <span className="text-lg font-bold text-accent mt-0.5">{curso.cantPracticasCalificadas} Prácticas</span>
                </div>
              </div>
                ...
                {curso.semanasExamenes.map((examen, idx) => (
                  <div key={idx} className="p-3 rounded-xl bg-rose-50/60 border border-rose-100 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs font-bold text-ink">{examen}</p>
